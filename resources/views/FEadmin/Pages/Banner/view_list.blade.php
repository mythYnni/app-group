...
@php
    ...
    $statusHTML = [
        0 => ['name' => 'Banner', 'color' => 'bg-cyan-900'],
        1 => ['name' => 'Popup', 'color' => 'bg-gray-800'],
    ];
@endphp
@section('css_view')
                        ...
                        <table id="res-config" class="display table table-striped table-hover dt-responsive nowrap"
                            ...
                            <tbody>
                                @foreach ($list as $key => $value)
                                    @php
                                        $type = $statusHTML[$value->status_type] ?? [
                                            'name' => 'Không Xác Định',
                                            'color' => '#000000',
                                        ];
                                        $typeName = $type['name'];
                                    ...
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $value->name }}</td>
                                        <td><a href="{{$value->link}}" class="badge rounded-pill {{ $typeColor }}">Linh Liên Kết</a></td>
                                        ...
                                        <td><span class="badge rounded-pill {{ $typeColor }}">{{ $typeName }}</span></td>
                                        <td class="action">
                                            <div class="btn-group-dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                        href="{{ route('delete_banner', $value->slug) }}" title="Delete"
                                                    ...
                                                    <a class="dropdown-item"
                                                        href="{{ route('view_update_banner', $value->slug) }}"><span
